...
def benchmark (extr = LocalBinaryPatterns(16, 3, cell_count = 8, patchSize= 30), sample_folder = "", sample_align_img = "", groundtruth_folder ="", predict_path = "", img_type = "*.bmp", \
    ...
    if extr is not None: 
        ...
        center_points, true_features = extract_samples_scale (sample_folder, extractor = extr, blur_size = BLUR_SIZE, scale= scale)
        ...
        for name in glob.glob(predict_path + img_type):#for each tif
            ...
            predict_scale (smp_img, name, center_points, true_features, window_size= WINDOW_SIZE, candidate_method = candidate_method, extractor= extr, mask_roi= mask, blur_size= BLUR_SIZE, open_kernel_size = OPEN_KERNEL_SIZE, scale=scale, debug= False)
...
sample_align_img = ""
predict_path = '../KeypointMatching/predict_sandfly/'
groundtruth_folder =  'G:\\My Drive\\Research\\iMorphSharedByHai\\Datasets\\Sandfly_data\\France_resized\\'

sample_folders = []
sample_folders.append('../KeypointMatching/training_sandfly_1')


extr = LocalBinaryPatterns(16, 3, cell_count = 8, patchSize= 30)
# for sample_folder in sample_folders:
#     benchmark (extr, sample_folder, sample_align_img, groundtruth_folder, predict_path, img_type = "*.tif", scale=0)

# extr = HOG(winSize = 64, blockSize =16,blockStride = 4, cellSize = 8)
# for sample_folder in sample_folders:
#     benchmark (extr, sample_folder, sample_align_img, groundtruth_folder, predict_path, img_type = "*.tif",  WINDOW_SIZE = 30, candidate_method = "keypoint_on_bin_img", scale=0)

#extr = StackedLocalBinaryPatterns(8, 3)    
for sample_folder in sample_folders:
    benchmark (extr, sample_folder, sample_align_img, groundtruth_folder, predict_path, img_type = "*.tif",  WINDOW_SIZE = 150, candidate_method = "keypoint_on_bin_img", scale=0)
